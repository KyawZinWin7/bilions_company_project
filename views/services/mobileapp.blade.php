      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-md-6 mt-4 ">
          <div class="mobileux-img">
            <a id="my-modal" class="my-modal-{{$image['id']}}" data-toggle="modal" data-target="#myModal{{$image['id']}}">
        ...
        <div class="modal fade" id="myModal{{$image['id']}}" role="dialog">
          <div class="modal-dialog modal-lg">
            <div class="modal-content ">
              ...
              <h4 class="ml-5"> Mobile App Development </h4>
              <h4 class="ml-5 lightbox-title ">{{ $image['title'] }}</h4>


              <div class="modal-body text-center">
                <div class="move-img">
                  <a class="prev" onclick="plusSlides(-1, '{{$image['id']}}')"><i class="fal fa-chevron-left"></i></a>
                  <a class="next" onclick="plusSlides(1, '{{$image['id']}}')"><i class="fal fa-chevron-right"></i></a>
                </div>
              ...
              <!-- ============== end slides image ============ -->
                  ...
                  <img src="{{$image['img']['image_'.$i]}}" class="img-fluid demo demo{{$image['id']}}" onclick="currentSlide({{$i}}, '{{$image['id']}}')" alt="">
                  @endfor
                  <!-- ============== end demo image ============ -->
                  ...
                  <div class="mt-3">
                  @for ($i = 1; $i <= count($image['img']); $i++) 
                  <span class="demo dot dot{{$image['id']}}" onclick="currentSlide({{$i}}, '{{$image['id']}}')"></span>
                  @endfor
                  <!-- ============== end dots ============ -->
                  </div>
              </div>
